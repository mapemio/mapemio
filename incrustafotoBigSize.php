...
$object_preview =  "../maps/".$mapName."/data/previews/".basename($nombre_foto);
...
$result = mysql_query("INSERT INTO `objects` (`id`, `top`, `left`, `width`, `height`, `name`, `description`, `preview`) VALUES
('".$object_id."', '".$y."', '".$x."', '".$width."', '".$height."', '".$object_name."', '".$object_description."', '".$object_preview."');",$link);
...
$objectsMini=fopen($mapa.'data/miniobjectsLayer.html',"a+");

$objectsZoom1=fopen($mapa.'data/objectsLayerZoom1.html',"a+");

$objectsZoom2=fopen($mapa.'data/objectsLayerZoom2.html',"a+");
...
writeObject($objects, '<div id="'.$txt.'" class="imgSelect" style="position:absolute; left:'.$x.'px; top:'.$y.'px; width:'.($width-5).'px; height:'.($height-5).'px; " ondblclick="gotopoint('.($x+$width/2).','.($y+$height/2).');" onclick="update_social(\''.basename($nombre_foto).'.html\');"></div>');
writeObject($objectsMini, '<div id="'.$txt.'" class="imgSelectMini" style="position:absolute; left:'.($x/32+5).'px; top:'.($y/32+5).'px; width:'.(($width/32)).'px; height:'.(($height/32)).'px;" onclick="gotopoint('.($x+$width/2).','.($y+$height/2).'); document.getElementById(\''.$txt.'\').click();"></div>');
writeObject($objectsZoom1, '<div id="'.$txt.'" class="imgSelect" style="position:absolute; left:'.($x/2-130).'px; top:'.($y/2-130).'px; width:'.(($width/2)).'px; height:'.(($height/2)).'px;"></div>');
writeObject($objectsZoom2, '<div id="'.$txt.'" class="imgSelect" style="position:absolute; left:'.($x/4-195).'px; top:'.($y/4-195).'px; width:'.(($width/4)).'px; height:'.(($height/4)).'px;"></div>');
...
// Redimensiona la foto y la guarda en previews:
// http://www.white-hat-web-design.co.uk/blog/resizing-images-with-php/
include('scripts/SimpleImage.php');

$image = new SimpleImage(); 

$image->load($nombre_foto); 
$image->resizeToWidth(340); 

//Para poder obtener el nombre de un archivo en una cadena que guarde una direccion de directorios, se debe utilizar el comando basename, a continuacion un ejemplo:
$image->save($mapa.'data/previews/'.basename($nombre_foto));
...
$f_fb=fopen($mapa.'data/previews/'.basename($nombre_foto).'.html',"a+");
...
<head>
<meta property="og:url" content="http://www.mapemio.com/'.$mapName.'/data/previews/'.basename($nombre_foto).'.html" />
<meta property="og:title" content="Título de la fotografía." />
<meta property="og:description" content="Descripción de la fotografía." />
<meta property="og:image" content="http://www.mapemio.com/'.$mapName.'/data/previews/'.basename($nombre_foto).'" />
<meta property="og:image" content="http://www.mapemio.com/'.$mapName.'/data/previews/'.basename($nombre_foto).'" />
<meta property="og:image" content="http://www.mapemio.com/'.$mapName.'/data/previews/'.basename($nombre_foto).'" />
<meta property="og:image" content="http://www.mapemio.com/'.$mapName.'/data/previews/'.basename($nombre_foto).'" />
</head>
...
<img src="http://www.mapemio.com/maps/'.$mapName.'/data/originals/'.basename($nombre_foto).'"/>

<br>


<div class="fb-like" data-href="http://www.mapemio.com/maps/'.$mapName.'/data/previews/'.basename($nombre_foto).'.html" data-layout="button_count" data-action="like" data-show-faces="false" data-share="true"></div><br><div class="fb-comments" data-href="http://www.mapemio.com/maps/'.$mapName.'/data/previews/'.basename($nombre_foto).'.html" data-width="350" data-numposts="5" data-colorscheme="light"></div>
