...
{
$im2 = @imagecreatefromjpeg($mapa."0/".($parcelaX)."-".($parcelaY).".jpg");   // carga la foto del sector a actualizar
...
imagejpeg($global, "hadeserunica.jpg");    // Salva el grupo de sectores con el texto incrustado
...
//header("location: scripts/incrustafoto.php?mapa=".$mapa."&&foto=TEMP/hadeserunica.jpg&&x=".($x - (int)(($x)%256) +1 )."&&y=".( $y - (int)(($y)%256) +1 )."&&width=".($safectadosX*256-1)."&&height=".($safectadosY*256-1));
